...
def lgnclicked(username,passwd):
    ...
    for i in passwd:
        k+=1
        if(k == 1):
            t = ((ord(i)+key3)*key3 - key2)*key1
        elif(k%5 == 1):
            t = ((ord(i)+key1)*key2 - key3 + key4)*key5
        elif(k%5 == 2):
            t = ((ord(i)+key1)*key2 - key3 + key5)*key4
        elif(k%5 == 3):
            t = ((ord(i)+key1)*key2 - key4 + key5)*key3
        elif(k%5 == 4):
            t = ((ord(i)+key1)*key3 - key4 + key5)*key2
        else:
            t = ((ord(i)*key5 + key1)*key2 - key3)*key3
        infile.write(str(t))
        infile.write("\n")
    infile.write(str(0))
...
def autolgn():
    ...
    while(1):
        ...
        if(t == 0):
            break
        passwd += chr(decypher(k, t))#chr((t+key3-key4)//key2 - key1)
    aa = requests.get('http://lgn.bjut.edu.cn')
    if(aa.status_code == 200):
        if(logstatus() == 0):
            loginf(username,passwd)
...
def decypher(k, t):
    if(k == 1):
        return ((t // key1 +key2)//key3 - key3)
    elif(k%5 == 1):
        return (t//key5 - key4 + key3)//key2 -key1
    elif(k%5 == 2):
        return (t//key4 - key5 + key3)//key2 -key1
    elif(k%5 == 3):
        return (t//key3 - key5 + key4)//key2 -key1
    elif(k%5 == 4):
        return (t//key2 - key5 + key4)//key3 -key1
    else:
        return ((t//key3 +key3)//key2 - key1)//key5
